<?php
// File: includes/shortcode-handler.php (v1.1.5 - Pagination + Timestamp Fix)
 ...
 * then relies on JS for subsequent background checks/refreshes and further pagination.
 */
function dsp_render_shortcode( $atts ) {
    ...
    // --- Fetch Initial Data (Page 1 ONLY) ---
    $items_per_page = defined('DSP_ITEMS_PER_PAGE') ? DSP_ITEMS_PER_PAGE : 25;
    $initial_db_data = DSP_DB_Handler::get_deals([ 'orderby' => 'first_seen', 'order' => 'DESC', 'items_per_page' => $items_per_page, 'page' => 1 ]);
    ...
    if ($deals_page_1) {
        foreach ($deals_page_1 as $deal) {
            if (is_object($deal) && isset($deal->first_seen) && isset($deal->title) && isset($deal->link)) {
                 $first_seen_ts = strtotime($deal->first_seen);
                 if ($first_seen_ts === false) $first_seen_ts = 0; // Handle parse error

                 $processed_deal = clone $deal; // Clone to avoid modifying original objects
    ...
    <div id="dsp-deal-display-container" class="dsp-container">
        <?php // Filters ?>
        <div class="dsp-filters"><div class="dsp-filter-item"><label for="dsp-search-input"><?php esc_html_e('Search:', 'deal-scraper-plugin'); ?></label><input type="text" id="dsp-search-input" placeholder="<?php esc_attr_e('Search...', 'deal-scraper-plugin'); ?>"></div><div class="dsp-filter-item dsp-filter-checkboxes"><span><?php esc_html_e('Sources:', 'deal-scraper-plugin'); ?></span><div id="dsp-source-checkboxes"></div></div><div class="dsp-filter-item dsp-filter-checkboxes"><label><input type="checkbox" id="dsp-new-only-checkbox"> <?php esc_html_e('New Only', 'deal-scraper-plugin'); ?></label></div><?php if ( $show_refresh_button ) : ?><div class="dsp-filter-item"><button id="dsp-refresh-button" class="dsp-button"><?php esc_html_e('Refresh Now', 'deal-scraper-plugin'); ?></button><span id="dsp-refresh-spinner" class="dsp-spinner" style="visibility: hidden;"></span><span id="dsp-refresh-message" class="dsp-refresh-status"></span></div><?php endif; ?></div>
        <?php // Controls ?>
        ...
        <?php // Status Bar ?>
        <div class="dsp-status-bar"><span id="dsp-status-message"><?php esc_html_e('Displaying deals. Checking for updates...', 'deal-scraper-plugin'); ?></span><span id="dsp-last-updated" style="float: right; visibility: <?php echo $last_fetch_time ? 'visible' : 'hidden'; ?>;"><?php esc_html_e('Last fetched:', 'deal-scraper-plugin'); ?> <span id="dsp-last-updated-time"><?php echo esc_html($last_fetch_display); ?></span></span></div>
        <?php // Table Wrapper ?>
                    ...
                    <?php if ( ! empty( $processed_deals_page_1 ) ) : ?>
                        <?php foreach ( $processed_deals_page_1 as $index => $deal ) : ?>
                            <?php
                               ...
                               // *** Use the added first_seen_ts property for data attribute ***
                               $first_seen_timestamp = $deal->first_seen_ts ?? 0;
                            ?>
                            <tr class="<?php echo esc_attr( implode(' ', $row_classes) ); ?>" data-source="<?php echo esc_attr( $deal->source ?? '' ); ?>" data-title="<?php echo esc_attr( $deal->title ?? '' ); ?>" data-description="<?php echo esc_attr( $deal->description ?? '' ); ?>" data-is-new="<?php echo esc_attr( $deal->is_new ? '1' : '0' ); ?>" data-first-seen="<?php echo esc_attr( $first_seen_timestamp ); ?>" data-price="<?php echo esc_attr( $sortable_price ); ?>" data-link="<?php echo esc_url( $deal->link ?? '#' ); ?>">
                        ...
                        <?php endforeach; ?>
                    <?php else : ?>
                        <tr class="dsp-no-deals-row"><td colspan="5"><?php esc_html_e('No deals found yet. Checking for updates...', 'deal-scraper-plugin'); ?></td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
