<?php
// File: admin/settings-page.php (v1.1.29 + Import Merge + Detailed Status + AJAX Source Actions)
...
function dsp_source_management_section_callback() {
    ...
    echo '<p>' . sprintf( wp_kses_post( __( 'Select the corresponding Parser File. Ensure the file exists in `%s` and contains the function `parse_%s_php()`. Changes to Enabled status, Parser File, and Delete actions save immediately.', 'deal-scraper-plugin' ) ), esc_html( str_replace(ABSPATH, '', DSP_PLUGIN_DIR) . 'includes/parsers/' ) ,'<i><parser_file_name></i>' ) . '</p>'; // Updated help text
    $available_parsers = [];
    ...
    <div id="dsp-source-manager-ui">
            ...
            <tbody id="dsp-sources-list">
                <?php if ( ! empty( $sites ) && is_array($sites) ) : ?>
                    <?php foreach ( $sites as $index => $site_data ) : ?>
                        ...
                        $site_name = $site_data['name'] ?? ''; $site_url = $site_data['url'] ?? ''; $parser_file = $site_data['parser_file'] ?? ''; $is_enabled = isset($site_data['enabled']) ? (bool)$site_data['enabled'] : false; $last_status = $site_data['last_status'] ?? ''; $last_run_time = isset($site_data['last_run_time']) ? (int)$site_data['last_run_time'] : 0; $status_class = ''; $status_display_text = $last_status;
                        ...
                        <tr class="dsp-source-row" data-index="<?php echo esc_attr( $index ); ?>">
                            ...
                            <td class="column-source-parser" data-label="<?php esc_attr_e( 'Parser File', 'deal-scraper-plugin' ); ?>">
                                ...
                                <span class="spinner dsp-input-spinner"></span> <?php // Spinner for parser save ?>
                                <span class="dsp-input-save-status"></span> <?php // Status message for parser save ?>
                            </td>
                            <td class="column-source-enabled" data-label="<?php esc_attr_e( 'Enabled', 'deal-scraper-plugin' ); ?>">
                                ...
                                <span class="spinner dsp-input-spinner"></span> <?php // Spinner for enabled save ?>
                                <span class="dsp-input-save-status"></span> <?php // Status message for enabled save ?>
                            </td>
                            <td class="column-source-status" data-label="<?php esc_attr_e( 'Last Status', 'deal-scraper-plugin' ); ?>">
                                <?php if ( $last_run_time > 0 ) : ?>
                                    ...
                                    <small class="dsp-status-time" title="<?php echo esc_attr(date('Y-m-d H:i:s', $last_run_time) . ' ' . wp_timezone_string()); ?>">
                                        <?php printf( esc_html__( '%s ago', 'deal-scraper-plugin' ), esc_html( human_time_diff( $last_run_time, current_time( 'timestamp' ) ) ) ); ?>
                                    </small>
                                <?php else : ?>
                            ...
                            <td class="column-source-actions" data-label="<?php esc_attr_e( 'Actions', 'deal-scraper-plugin' ); ?>">
                                ...
                                <span class="spinner dsp-test-source-spinner"></span> <?php // Existing spinner for test ?>
                                <div class="dsp-test-source-result"></div> <?php // Existing result div for test ?>
                                <span class="spinner dsp-delete-spinner"></span> <?php // Spinner for delete action ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else : ?>
        ...
        <template id="dsp-source-row-template">
             <?php // Note: Added spinners/status spans to template as well ?>
            <tr class="dsp-source-row" data-index="__INDEX__">
                ...
                <td class="column-source-parser" data-label="<?php esc_attr_e( 'Parser File', 'deal-scraper-plugin' ); ?>">
                    ...
                    <span class="spinner dsp-input-spinner"></span>
                    <span class="dsp-input-save-status"></span>
                </td>
                <td class="column-source-enabled" data-label="<?php esc_attr_e( 'Enabled', 'deal-scraper-plugin' ); ?>">
                ...
                <td class="column-source-actions" data-label="<?php esc_attr_e( 'Actions', 'deal-scraper-plugin' ); ?>">
                    <button type="button" class="button button-secondary dsp-test-source-button" title="<?php esc_attr_e( 'Test fetching and parsing this source', 'deal-scraper-plugin'); ?>"><?php esc_html_e( 'Test', 'deal-scraper-plugin' ); ?></button>
                    <button type="button" class="button button-link-delete dsp-delete-source-button" style="margin-left: 5px;"><?php esc_html_e( 'Delete', 'deal-scraper-plugin' ); ?></button>
                    <span class="spinner dsp-test-source-spinner"></span>
                    <div class="dsp-test-source-result"></div>
...
function dsp_sanitize_settings( $input ) {
    // Sanitize logic remains the same as previous version
    // Get current settings *before* they are updated
    $old_options = get_option( DSP_OPTION_NAME, [] );
    if ( ! is_array( $old_options ) ) { $old_options = []; }
    $output = $old_options; $defaults = dsp_get_default_config(); $import_processed = false; $sources_changed = false;
    // --- Handle File Import ---
    if ( isset( $_FILES['dsp_import_file'] ) && !empty( $_FILES['dsp_import_file']['tmp_name'] ) && $_FILES['dsp_import_file']['error'] === UPLOAD_ERR_OK ) {
        if (!current_user_can('manage_options')) { add_settings_error('dsp_import_file', 'dsp_import_permission_error', __('Permission denied for import.', 'deal-scraper-plugin'), 'error'); }
        else {
            $file = $_FILES['dsp_import_file']; $filename = $file['name']; $tmp_name = $file['tmp_name']; $file_type = $file['type']; $file_ext = strtolower( pathinfo( $filename, PATHINFO_EXTENSION ) ); $allowed_types = ['application/json', 'application/zip', 'application/octet-stream']; $allowed_exts = ['json', 'zip'];
            if ( ! in_array( $file_ext, $allowed_exts ) || ! in_array( $file_type, $allowed_types ) ) { add_settings_error('dsp_import_file', 'dsp_import_type_error', __('Invalid file type. Please upload a .json or .zip file.', 'deal-scraper-plugin'), 'error'); }
            elseif ( $file['size'] > 5 * MB_IN_BYTES ) { add_settings_error('dsp_import_file', 'dsp_import_size_error', __('File is too large (max 5MB).', 'deal-scraper-plugin'), 'error'); }
            else {
                $json_content = false; $import_source_type = 'json';
                if ($file_ext === 'zip') { $import_source_type = 'zip'; if ( ! class_exists( 'ZipArchive' ) ) { add_settings_error('dsp_import_file', 'dsp_import_zip_error', __('ZipArchive PHP extension not available. Cannot process .zip files.', 'deal-scraper-plugin'), 'error'); } else { $zip = new ZipArchive(); $res = $zip->open($tmp_name); if ($res === true) { $json_index = $zip->locateName('sources.json', ZipArchive::FL_NOCASE); if ($json_index !== false) { $json_content = $zip->getFromIndex($json_index); } else { add_settings_error('dsp_import_file', 'dsp_import_zip_content_error', __('Could not find `sources.json` inside the Zip archive.', 'deal-scraper-plugin'), 'error'); } $zip->close(); } else { add_settings_error('dsp_import_file', 'dsp_import_zip_open_error', __('Could not open the Zip archive.', 'deal-scraper-plugin'), 'error'); } } }
                else { $json_content = file_get_contents( $tmp_name ); if ( $json_content === false ) { add_settings_error('dsp_import_file', 'dsp_import_read_error', __('Could not read uploaded JSON file.', 'deal-scraper-plugin'), 'error'); } }
                if ($json_content !== false) {
                    $imported_sites = json_decode( $json_content, true );
                    if ( $imported_sites === null && json_last_error() !== JSON_ERROR_NONE ) { add_settings_error('dsp_import_file', 'dsp_import_json_error', __('Invalid JSON format: ', 'deal-scraper-plugin') . json_last_error_msg(), 'error'); }
                    elseif ( !is_array($imported_sites) ) { add_settings_error('dsp_import_file', 'dsp_import_format_error', __('Invalid format: Imported data is not an array of sources.', 'deal-scraper-plugin'), 'error'); }
                    else {
                         $validated_imported_sites = []; $has_invalid_entry = false; $parser_errors = []; $available_parsers = []; $parser_dir = DSP_PLUGIN_DIR . 'includes/parsers/'; if ( is_dir( $parser_dir ) && is_readable( $parser_dir ) ) { $parser_files = glob( $parser_dir . '*.php' ); if ( $parser_files ) { foreach ( $parser_files as $parser_file_path ) { $base_name = basename( $parser_file_path, '.php' ); if ( !empty($base_name) && strpos($base_name, '.') === false ) { $available_parsers[] = $base_name; } } } }
                         foreach ($imported_sites as $index => $site_data) { $is_entry_valid = true; $entry_error = ''; if ( !is_array( $site_data ) || empty($site_data['name']) || !is_string($site_data['name']) || empty($site_data['url']) || !is_string($site_data['url']) || filter_var($site_data['url'], FILTER_VALIDATE_URL) === false || empty($site_data['parser_file']) || !is_string($site_data['parser_file']) ) { $is_entry_valid = false; $entry_error = "Missing/invalid basic fields"; } elseif ($import_source_type === 'json' && !in_array($site_data['parser_file'], $available_parsers, true) ) { $is_entry_valid = false; $entry_error = "Parser file '{$site_data['parser_file']}.php' not found locally"; $parser_errors[] = $site_data['parser_file']; } if (!$is_entry_valid) { $has_invalid_entry = true; error_log("DSP Import Warning: Skipping invalid site entry at index {$index} ({$entry_error}): " . print_r($site_data, true)); continue; } $validated_imported_sites[] = [ 'name' => sanitize_text_field( $site_data['name'] ), 'url' => esc_url_raw( $site_data['url'] ), 'parser_file' => sanitize_key( $site_data['parser_file'] ), 'enabled' => isset($site_data['enabled']) ? (bool)$site_data['enabled'] : false, 'last_status' => '', 'last_run_time' => 0 ]; }
                        if (!empty($validated_imported_sites)) {
                            ...
                            if ( $import_mode === 'merge' ) {
                                $current_sources = $output['sites'] ?? []; $current_source_names = array_map('strtolower', array_column($current_sources, 'name')); $added_count = 0; $skipped_count = 0;
                                foreach ($validated_imported_sites as $imported_site) { if (!in_array(strtolower($imported_site['name']), $current_source_names, true)) { $current_sources[] = $imported_site; $added_count++; } else { $skipped_count++; } }
                                $output['sites'] = $current_sources; $sources_changed = ($added_count > 0);
                                $merge_msg = sprintf( _n( 'Merge complete. Added %d new source.', 'Merge complete. Added %d new sources.', $added_count, 'deal-scraper-plugin' ), $added_count ); if ($skipped_count > 0) { $merge_msg .= ' ' . sprintf( _n( 'Skipped %d duplicate source (by name).', 'Skipped %d duplicate sources (by name).', $skipped_count, 'deal-scraper-plugin' ), $skipped_count ); } if ($has_invalid_entry) { $merge_msg .= ' ' . __('Some invalid entries in the file were also skipped.', 'deal-scraper-plugin'); } if ($import_source_type === 'zip') { $merge_msg .= ' ' . __('Remember to manually upload parser files.', 'deal-scraper-plugin'); } add_settings_error('dsp_import_file', 'dsp_import_merged', $merge_msg, $has_invalid_entry || $skipped_count > 0 ? 'warning' : 'updated');
                            } else {
                                $output['sites'] = $validated_imported_sites; $sources_changed = true;
                                $overwrite_msg = sprintf( _n( 'Successfully imported and overwrote configuration with %d source.', 'Successfully imported and overwrote configuration with %d sources.', count($validated_imported_sites), 'deal-scraper-plugin' ), count($validated_imported_sites) ); if ($has_invalid_entry) { $overwrite_msg .= ' ' . __('Some invalid entries were skipped.', 'deal-scraper-plugin'); } if ($import_source_type === 'zip') { $overwrite_msg .= ' ' . __('Remember to manually upload parser files.', 'deal-scraper-plugin'); } add_settings_error('dsp_import_file', 'dsp_import_overwritten', $overwrite_msg, $has_invalid_entry ? 'warning' : 'updated');
                            }
                        } elseif ($has_invalid_entry) { add_settings_error('dsp_import_file', 'dsp_import_all_invalid', __('Import failed. All entries in the file were invalid or unusable.', 'deal-scraper-plugin'), 'error'); } else { add_settings_error('dsp_import_file', 'dsp_import_empty', __('Import file was valid but contained no source entries.', 'deal-scraper-plugin'), 'warning'); } if (!empty($parser_errors)) { add_settings_error('dsp_import_file', 'dsp_import_parser_missing', __('Warning: The following required parser files were not found locally: ', 'deal-scraper-plugin') . implode(', ', array_unique($parser_errors)) . '.php', 'warning'); }
    ...
    // --- Sanitize Manually Edited/Added Sources ---
    if (!$import_processed && isset($input['sites'])) {
        $current_sites = $old_options['sites'] ?? []; $current_sites_map = []; if (is_array($current_sites)) { foreach ($current_sites as $index => $site) { if (is_array($site) && isset($site['name'])) {$current_sites_map[strtolower($site['name'])] = $site; } } }
        $sanitized_sites = []; $original_indices = array_keys($input['sites']); $has_manual_error = false;
