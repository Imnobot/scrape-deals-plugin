 ...
 * Version:           1.1.24 // Fix 'New Only' filter with staggered cron
 * Author:            ᕦ(ò_óˇ)ᕤ
...
define( 'DSP_VERSION', '1.1.24' ); // Use this for assets & DB version check
...
function dsp_enqueue_admin_assets( $hook ) { if ( 'settings_page_deal_scraper_settings' !== $hook ) { return; } $plugin_version = defined('DSP_VERSION') ? DSP_VERSION : '1.1.24'; wp_enqueue_script( 'dsp-admin-script', DSP_PLUGIN_URL . 'assets/js/admin-settings.js', ['jquery', 'wp-util'], $plugin_version, true ); wp_localize_script( 'dsp-admin-script', 'dsp_admin_ajax_obj', [ 'ajax_url' => admin_url( 'admin-ajax.php' ), 'nonce' => wp_create_nonce( 'dsp_admin_ajax_nonce' ), 'sending_text' => __( 'Sending...', 'deal-scraper-plugin' ), 'error_text' => __( 'An error occurred.', 'deal-scraper-plugin' ), 'testing_text' => __( 'Testing...', 'deal-scraper-plugin' ), 'test_error_text' => __( 'Test failed. Check inputs.', 'deal-scraper-plugin' ), 'delete_confirm_text' => __( 'Are you sure you want to delete this source?', 'deal-scraper-plugin' ), ] ); $admin_css_path = DSP_PLUGIN_DIR . 'assets/css/admin-style.css'; if (file_exists($admin_css_path)) { wp_enqueue_style( 'dsp-admin-style', DSP_PLUGIN_URL . 'assets/css/admin-style.css', [], $plugin_version ); } }

// --- Common Deal Processing Function ---
/** Processes raw deal objects for AJAX responses. */
function dsp_process_deals_for_ajax( $deals, $last_fetch_time ) { $processed_deals = []; if ( empty($deals) || !is_array($deals) ) { return $processed_deals; } $date_format = get_option('date_format'); $time_format = get_option('time_format'); foreach ( $deals as $deal ) { if ( is_object($deal) && isset($deal->first_seen) ) { $first_seen_ts = strtotime($deal->first_seen); if ($first_seen_ts === false) $first_seen_ts = 0; $processed_deal = clone $deal; // Use the passed $last_fetch_time to determine 'is_new' for this specific request context
            $processed_deal->is_new = ($first_seen_ts && $last_fetch_time && $first_seen_ts >= $last_fetch_time); $processed_deal->first_seen_formatted = $first_seen_ts ? date_i18n( "{$date_format} {$time_format}", $first_seen_ts ) : 'N/A'; $processed_deal->first_seen_ts = $first_seen_ts; $processed_deal->is_lifetime = isset($deal->is_ltd) ? (bool)$deal->is_ltd : dsp_is_lifetime_deal_php($deal); $processed_deals[] = $processed_deal; } } return $processed_deals; }
...
function dsp_ajax_get_deals_handler() {
    ...
    $min_price_input = isset($_POST['min_price']) ? trim(wp_unslash($_POST['min_price'])) : ''; $max_price_input = isset($_POST['max_price']) ? trim(wp_unslash($_POST['max_price'])) : ''; $min_price = ($min_price_input !== '' && is_numeric($min_price_input) && $min_price_input >= 0) ? (float) $min_price_input : null; $max_price = ($max_price_input !== '' && is_numeric($max_price_input) && $max_price_input >= 0) ? (float) $max_price_input : null;

    // Prepare args for DB handler
    $db_args = [ 'orderby' => $orderby, 'order' => $order, 'items_per_page' => $items_per_page, 'page' => $page, 'search' => $search_term, 'sources' => $sources, 'min_price' => $min_price, 'max_price' => $max_price, 'ltd_only' => $ltd_only ];

    // *** MODIFIED: Get last successful run time for 'New Only' filter ***
    ...
    if ($new_only) {
        ...
        // If $last_successful_run_time is 0, the filter won't be applied (correct behavior)
    }

    // Fetch Deals (DB Handler handles LTD and potentially 'newer_than_ts')
    $db_result = DSP_DB_Handler::get_deals($db_args);
    $fetched_deals = $db_result['deals'] ?? [];

    // Process Deals for Display (Pass the correct timestamp for 'is_new')
    // Use $last_successful_run_time if 'new_only' was checked, otherwise 0 (or legacy time?)
    // Let's use $last_successful_run_time consistently for the 'is_new' flag context in this request.
    ...
    // Get the most recent success time again for the display string
    $display_last_fetch_time = dsp_get_last_successful_run_timestamp(); // Use helper for display time too
    ...
    wp_send_json_success( [
        ...
        'current_page' => $page,
        // Use the calculated last *successful* run time for the display
