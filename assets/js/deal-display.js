// File: assets/js/deal-display.js (v1.1.6 - Integrate Filters with Pagination - Verified Complete)
...
jQuery(document).ready(function ($) {
    ...
    let isCheckingForDeals = false; let isRefreshing = false; let isSubscribing = false; let isLoadingPage = false;
    ...
    function init() {
        ...
        currentPage = 1;
        applyInitialDarkMode(); createSourceCheckboxes(); parseInitialTable(); renderPagination(); bindEvents(); checkForNewDeals(); updateStatusMessage();
    }
    ...
    function parseInitialTable() {
        ...
        tableBody.find('tr.dsp-deal-row').each(function() {
            ...
            const deal = {
                ...
                is_new: row.data('is-new') == '1', is_lifetime: row.hasClass('dsp-lifetime-item'),
             };
            if (deal.link !== '#' && deal.title && deal.first_seen_ts > 0) { allDealsData.push(deal); } else { console.warn("DSP: Skipped parsing row:", row, deal); }
        });
        if (allDealsData.length > 0) tableBody.find('.dsp-no-deals-row').remove();
        console.log(`DSP: Parsed ${allDealsData.length} initial deals from PHP.`);
    }

    /** Background AJAX check for new deals */
    function checkForNewDeals() {
        if (isCheckingForDeals || isRefreshing) { return; } isCheckingForDeals = true; statusMessage.text(__(dsp_ajax_obj.checking_text)).removeClass('dsp-error dsp-success'); console.log("DSP: Starting background check...");
        $.ajax({
            url: dsp_ajax_obj.ajax_url, type: 'POST', data: { action: 'dsp_check_for_new_deals', nonce: dsp_ajax_obj.nonce, page: currentPage, items_per_page: itemsPerPage, orderby: currentSort.key, order: currentSort.reverse ? 'DESC' : 'ASC' }, timeout: 180000,
            success: function (response) { console.log("DSP: Background check response:", response); if (response.success && response.data) { totalItems = parseInt(response.data.total_items, 10) || 0; renderPagination(); const dealsFromServer = response.data.deals || []; if (parseInt(response.data.current_page, 10) === currentPage) { console.log("DSP: Background check returned data for current page. Updating table."); allDealsData = dealsFromServer; renderTable(allDealsData); } else { console.log("DSP: Background check response was not for current page."); } updateLastUpdated(response.data.last_fetch); statusMessage.text(response.data.message || __(dsp_ajax_obj.check_complete_text)).removeClass('dsp-error').addClass('dsp-success'); updateStatusMessage(); } else { console.error("DSP Error checking deals:", response); const errorMsg = response.data?.message || __(dsp_ajax_obj.error_check_text); statusMessage.text(errorMsg).removeClass('dsp-success').addClass('dsp-error'); } },
            error: function (jqXHR, textStatus, errorThrown) { console.error("DSP AJAX Check Error:", textStatus, errorThrown, jqXHR.responseJSON); let errorMsg = __(dsp_ajax_obj.error_check_ajax_text); if (jqXHR.responseJSON?.data?.message) { errorMsg = jqXHR.responseJSON.data.message; } statusMessage.text(errorMsg).removeClass('dsp-success').addClass('dsp-error'); },
            complete: function () { isCheckingForDeals = false; console.log("DSP: Background check complete."); if (!statusMessage.hasClass('dsp-error')) { updateStatusMessage(); } }
        });
    }
    ...
    function fetchDealsPage(pageNum) {
        ...
        const searchTerm = searchInput.val().trim(); // Keep original case? Backend handles LIKE %term%
        ...
        console.log(`DSP: Fetching page ${currentPage}... Sort: ${sortKey} ${sortReverse ? 'DESC' : 'ASC'}, Search: "${searchTerm}", New: ${showNew}, Sources: ${activeSources.join(',')}`);
        const requestData = {
            action: 'dsp_get_deals', nonce: dsp_ajax_obj.nonce, page: currentPage, items_per_page: itemsPerPage,
            orderby: sortKey, order: sortReverse ? 'DESC' : 'ASC',
            // *** ADDED Filter data to request ***
            search: searchTerm,
            sources: activeSources, // Send as an array
            new_only: showNew ? 1 : 0, // Send 1 or 0
        };
        $.ajax({
            url: dsp_ajax_obj.ajax_url, type: 'POST', data: requestData, timeout: 30000,
            success: function(response) { console.log("DSP: fetchDealsPage response:", response); if (response.success && response.data) { allDealsData = response.data.deals || []; totalItems = parseInt(response.data.total_items, 10) || 0; itemsPerPage = parseInt(response.data.items_per_page, 10) || itemsPerPage; currentPage = parseInt(response.data.current_page, 10) || currentPage; renderTable(allDealsData); renderPagination(); } else { console.error("DSP: Error fetching deals page:", response); tableBody.html('<tr class="dsp-error-row"><td colspan="5">' + __(dsp_ajax_obj.error_text) + '</td></tr>'); statusMessage.text(response.data?.message || __(dsp_ajax_obj.error_text)).addClass('dsp-error'); renderPagination(); } },
            error: function(jqXHR, textStatus, errorThrown) { console.error("DSP: AJAX fetchDealsPage Error:", textStatus, errorThrown); tableBody.html('<tr class="dsp-error-row"><td colspan="5">' + __(dsp_ajax_obj.error_text) + ' (AJAX)</td></tr>'); statusMessage.text(__(dsp_ajax_obj.error_text) + ' (AJAX)').addClass('dsp-error'); renderPagination(); },
            complete: function() { isLoadingPage = false; paginationControlsContainer.removeClass('dsp-loading'); if (!statusMessage.hasClass('dsp-error')) { updateStatusMessage(); } }
        });
    }
    ...
    function createDealRowElement(deal) {
        if (!deal || !deal.title || !deal.link || !deal.first_seen_formatted) { console.warn("DSP: Cannot create row for invalid deal:", deal); return $(); }
        const isNew = deal.is_new || false; const isLifetime = deal.is_lifetime || false;
        const firstSeenTimestamp = parseInt(deal.first_seen_ts, 10) || 0; // Use timestamp
        const row = $('<tr></tr>').addClass('dsp-deal-row').attr({ 'data-source': deal.source || '', 'data-title': deal.title || '', 'data-description': deal.description || '', 'data-is-new': isNew ? '1' : '0', 'data-first-seen': firstSeenTimestamp, 'data-price': parsePriceForSort(deal.price || ''), 'data-link': deal.link || '#' });
    ...
    function renderTable(dealsToDisplay) { const noDealsText = __(dsp_ajax_obj.no_deals_found_text); tableBody.empty(); if (!dealsToDisplay || dealsToDisplay.length === 0) { const noDealsRow = $('<tr></tr>').addClass('dsp-no-deals-row').append( $('<td colspan="5"></td>').text(noDealsText) ); tableBody.html(noDealsRow); return; } dealsToDisplay.forEach((deal, index) => { const rowElement = createDealRowElement(deal); rowElement.addClass(index % 2 === 0 ? 'dsp-even-row' : 'dsp-odd-row'); tableBody.append(rowElement); }); }
    ...
    function getSortValue(deal, key) { if (!deal) return ''; switch (key) { case 'is_new': return deal.is_new ? 1 : 0; case 'title': return (deal.title || '').toLowerCase(); case 'price': return parsePriceForSort(deal.price || ''); case 'source': return (deal.source || '').toLowerCase(); case 'first_seen': return parseInt(deal.first_seen_ts, 10) || 0; default: return ''; } } // Use timestamp
    ...
    /** Simple translation placeholder - Improved Fallback */
     ...
     function updateStatusMessage() { if (isLoadingPage || isCheckingForDeals || isRefreshing || isSubscribing) return; const dealsOnPage = allDealsData.length; const totalDealCount = totalItems; const totalPages = itemsPerPage > 0 ? Math.ceil(totalDealCount / itemsPerPage) : 1; let statusText = ''; if (totalDealCount > 0 && itemsPerPage > 0) { const firstItem = (currentPage - 1) * itemsPerPage + 1; const lastItem = Math.min(currentPage * itemsPerPage, totalDealCount); statusText = sprintf(__('Showing deals %d-%d of %d', 'deal-scraper-plugin'), firstItem, lastItem, totalDealCount); if (totalPages > 1) { statusText += ` (${sprintf(__(dsp_ajax_obj.page_text), currentPage, totalPages)})`; } } else if (totalDealCount > 0 && itemsPerPage <= 0) { statusText = sprintf(__('Showing %d deals', 'deal-scraper-plugin'), totalDealCount); } else { statusText = __(dsp_ajax_obj.no_deals_found_text); } const searchTerm = searchInput.val().trim(); const showNew = newOnlyCheckbox.is(':checked'); const activeSources = container.find('.dsp-source-filter-cb:checked').map(function() { return $(this).val(); }).get(); const totalSources = dsp_ajax_obj.config_sources?.length || 0; const filterParts = []; if (searchTerm) filterParts.push(sprintf('%s: \'%s\'', __('Search'), searchTerm)); if (totalSources > 0 && activeSources.length < totalSources) filterParts.push(__('Sources filtered')); if (showNew) filterParts.push(__('New only')); if (filterParts.length > 0) { statusText += ` | ${__('Filters')}: ${filterParts.join(', ')}`; } statusMessage.text(statusText).removeClass('dsp-error dsp-success'); }
